...
void Renderer::initVulkan(std::span<const char*> instance_extensions)
{
  ...
  deviceExtensions.push_back("VK_EXT_dynamic_rendering_unused_attachments");

  auto dynamicRenderingUnusedAttachmentsFeatures =
  ...
  vk12Features.pNext = &dynamicRenderingUnusedAttachmentsFeatures;
...
void Renderer::drawFrame()
{
  ...
  if (nextSwapchainImage)
  {
    ...
    ETNA_CHECK_VK_RESULT(currentCmdBuf.begin(vk::CommandBufferBeginInfo{}));
    {
      ...
      if (worldRenderer->FXAA_ON)
      {
        worldRenderer->renderWorldFXAA(currentCmdBuf, image, view);
      }
      else
      {
